-- Extend save_quote_draft_children to persist paint config and line-item product refs
...
-- Drop old 4-arg overload so callers must use the new signature
drop function if exists public.save_quote_draft_children(uuid, jsonb, jsonb, jsonb, jsonb);

grant execute on function public.save_quote_draft_children(uuid, jsonb, jsonb, jsonb, jsonb, jsonb)
  to authenticated;