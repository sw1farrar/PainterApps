-- Per-surface paint products, quote-level defaults, and customer pricing on products

-- ---------------------------------------------------------------------------
-- Company paint products: what you charge the customer (per gallon)
-- ---------------------------------------------------------------------------
...
comment on column public.company_paint_products.unit_cost is
  'What the company pays per gallon (your cost).';

comment on column public.company_paint_products.unit_price is
  'What the company charges the customer per gallon (sell price).';

comment on column public.company_paint_products.gallons_per_labor_hour is
  'Optional override: gallons this product covers per crew hour. Falls back to company default.';

-- ---------------------------------------------------------------------------
-- Quote-level paint defaults (cascade to all areas unless overridden)
-- ---------------------------------------------------------------------------

create table if not exists public.quote_paint_defaults (
  id uuid primary key default gen_random_uuid(),
  quote_id uuid not null references public.quotes(id) on delete cascade,
  surface_type public.quote_surface_kind not null,
  company_paint_product_id uuid references public.company_paint_products(id) on delete set null,
  coats integer not null default 2,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint quote_paint_defaults_unique_surface
    unique (quote_id, surface_type)
);

create index if not exists quote_paint_defaults_quote_id_idx
  on public.quote_paint_defaults (quote_id);

alter table public.quote_paint_defaults enable row level security;
...
create policy quote_paint_defaults_all on public.quote_paint_defaults
  ...
  using (
    exists (
      ...
      where q.id = quote_paint_defaults.quote_id
...
grant select, insert, update, delete
  on table public.quote_paint_defaults
  to authenticated;

grant all on table public.quote_paint_defaults to service_role;

-- ---------------------------------------------------------------------------
-- Per-surface product assignment on quote areas
-- ---------------------------------------------------------------------------
...
alter table public.quote_surfaces
  add column if not exists product_override boolean not null default false;

alter table public.quote_surfaces
  add column if not exists gallons_estimated numeric;

comment on column public.quote_surfaces.product_override is
  'When true, quote_paint_defaults changes do not overwrite this surface product.';

comment on column public.quote_surfaces.gallons_estimated is
  'Cached decimal gallon estimate for this surface (includes waste factor).';

-- ---------------------------------------------------------------------------
-- Extend save_quote_draft_children for paint defaults + surface products
-- ---------------------------------------------------------------------------

drop function if exists public.save_quote_draft_children(uuid, jsonb, jsonb, jsonb, jsonb, jsonb);
