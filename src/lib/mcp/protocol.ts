    ...
    if (method === "initialize") {
      ...
      return {
        ...
        result: {
          ...
          instructions:
            "You are connected to the signed-in user's PainterApps company. Use get_paintday for weather. Use list_manufacturers, list_products, get_product, and update_product for the live TDS catalog (editors / platform_admin on write). The Products page does not calculate quality. You set quality_score (0–10, one decimal, 10 best) and quality_summary (one sentence) after researching that product against others in the same application_class: exterior-topcoat, interior-topcoat, trim, floor, or primer. A wide application window or early moisture resistance is a capability, not an automatic higher score. list_products is ordered like the page: same job together, highest stored score first, unrated products last. get_product returns quality.score and quality.summary from those columns. On each product set description, features[], benefits[], and a can photo via can_image_url or can_image_base64 (PNG/JPEG/WebP). Odd keys go in attrs. Do not invent product numbers or solids. official_tds_pdf_url or a stored PDF powers the full-size PDF preview. Systems page filters are interior/exterior, application_types, substrates, sheen, VOC, manufacturer. Call list_application_types then update_system with only { id, application_types } to tag uses. Use list_tds_documents / create_tds_document for PDF sheets (raw_text for extracted TDS). Use suggest_systems with a ZIP so recommendations follow today's conditions.",
        },
      };
    }
