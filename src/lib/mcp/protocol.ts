    ...
    if (method === "initialize") {
      ...
      return {
        ...
        result: {
          ...
          instructions:
            "You are connected to the signed-in user's PainterApps company. Use get_paintday for weather. Use list_manufacturers, list_products, get_product, and update_product for the live TDS catalog (editors / platform_admin on write). list_products is ordered like the Products page: same-job groups, highest quality first. Each product includes a computed quality object (score 0–10, application_class, resin, solids_pct, claims). Quality is not stored — set volume_solids_pct, weight_solids_pct, resin_type, description, features[], and benefits[] and the score updates. Do not write quality or quality_score. application_class is exterior-topcoat, interior-topcoat, trim, floor, or primer. On each product set description, features[], benefits[], and a can photo via can_image_url or can_image_base64 (PNG/JPEG/WebP). Odd keys go in attrs. Do not invent product numbers. official_tds_pdf_url or a stored PDF powers the full-size PDF preview. Systems page filters are interior/exterior, application_types, substrates, sheen, VOC, manufacturer. Call list_application_types then update_system with only { id, application_types } to tag uses. Use list_tds_documents / create_tds_document for PDF sheets (raw_text for extracted TDS). Use suggest_systems with a ZIP so recommendations follow today's conditions.",
        },
      };
    }
